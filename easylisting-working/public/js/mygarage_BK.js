...
// 				$("#description_en").tex('');
				...
				$.each($gallery,function (i,v) {
						...
						$galID = v.id;
						 ...
						 $this.parent().children('img').attr('src', '/uploaded/post/'+$gal.post_id+'/gallery/160x100-'+$gal.name).attr('data-post_id',$gal.id);
		  		      	$($uploadBtn).empty();
		  		        ...
		  		        $('<button/>').addClass('btn btn-danger btn-remove').append('<i class="glyphicon glyphicon-remove"></i>')
		  		            ...
		  		            .click(function () {
								$g = $gallery[i];
					  			$thisBtn = $(this);
					   			$upload = $thisBtn.parent();
								$span = $thisBtn.parent().parent().children('span')[0];

								$img = $($span).children('img');
		  		                //if(confirm('r u sure?')){
									
		  						  $.get('/post/delete-gallery/'+$g.post_id+'/'+$g.id,function(){
		  			                  $($upload).empty();

									  $img.attr('src', '/img/car-empty.jpg');
		  						  });
				...
				// for preview
				// $("#preview-price").html($("#submit-price").val());
// 				$("#preview-price").html($("#submit-price").val());
//
// 				$.each($gallery, function( idx, gal ) {
//
// 				});
...
// 			    }
    ...
    $("#preview-gallery").html('');
		...
		$("#preview-title").text($( "#make option:selected" ).text() + ' ' + $( "#model option:selected" ).text() + ' ' + $( "#submodel option:selected" ).text() + ' ' + $( "#year option:selected" ).text());
		...
		$("#preview-detail").text($("#description_en").text());
    ...
    if ($("span.fileinput-button").length > 1) {

...
class AdminPromoteController extends \AdminController 
{
    ...
    var $img_root_url;
    var $img_root_path = 'uploaded/promote/';
    ...
    public function getIndex()
    {
        $q = Input::get('q','');
        $slug = Input::get('slug','');
        $pages = Promote::with('lang');
        $allowed_sort = ['created_at'];
        $sort = in_array(Input::get('sort'), $allowed_sort) ? Input::get('sort') : 'promotes.created_at';
        $order = Input::get('order') === 'asc' ? 'asc' : 'desc';

        $pages = Promote::select('promotes.*','promotes_languages.title as title')
                ->join('promotes_languages', 'promotes_languages.promote_id', '=', 'promotes.id')
                ->where('status', '!=', $this->status['deleted'])
                ->groupby('promotes_languages.promote_id')
                ->with('post_by');

        if($q!='') {
            $pages = $pages->where('promotes_languages.title', 'LIKE', "%$q%")->groupby('promotes_languages.promote_id');
        }
        if($slug!='') {
            $pages = $pages->where('promotes.slug', 'LIKE', "%$slug%");
        }
        ...
        return View::make('admin.promote.index');
    }
    ...
    public function postStore()
    {
        ...
        if ( ! $validator->fails())
        {
            ...
            $lastinsert_id = $p->id;
            foreach($languages as $lang) {
                PromoteLanguage::create(array(
                    'promote_id'       => $lastinsert_id,
                    'title'         => $inputs['title_'.$lang->short_code],
                    'language_id'   => $lang->id
                ));
            }

            Notification::success('The promote was saved.');
            return Redirect::to('admin/content-promote/edit/'.$lastinsert_id);
        } else {
    ...
    public function getEdit($id)
    {
        ...
        return View::make('admin.promote.edit');
    }
        ...
        if ( ! $validator->fails())
        {
            ...
            foreach($languages as $lang){
                if (Input::has('title_'.$lang->short_code))
                {
                    ...
                    PromoteLanguage::where('promote_id','=',$id)
            ...
            return Redirect::to('admin/content-promote/edit/'.$id);
        } else {
            foreach ($validator->messages()->toArray() as $key => $value) {
                Notification::warning($value);
            }
    ...
    public function postFileThumbnail()
    {
        ...
        if (Input::hasFile($file_input_name))
        {
            ...
            $results = array(
                ...
                'size'          => $filesize,
                'deleteType'    =>'get',
                'deleteUrl'     => URL::to('admin/content-promote/delete-thumbnail/'.$id)
            );
            $res[] = $results;
